...
#[test]
fn encrypted_workspace_rejects_oversized_generated_metadata_and_journals() {
    let workdir = temporary_directory("generated-control-limits");
    let root = workdir.join("fs");
    std::fs::create_dir_all(&root).unwrap();

    let oversized_key = KeyMetadata {
        version: KEY_METADATA_VERSION,
        salt: base64::engine::general_purpose::STANDARD.encode([0_u8; 16]),
        key_id: "x".repeat(super::MAX_KEY_METADATA_BYTES),
    };
    assert!(
        EncryptedWorkspace::write_key_metadata(&root, &oversized_key)
            .unwrap_err()
            .to_string()
    ...
    let too_many = RekeyJournal {
        version: REKEY_JOURNAL_VERSION,
        old_key: metadata.clone(),
        new_key: metadata.clone(),
        entries: std::iter::repeat_with(|| RekeyEntry {
            destination: "destination".to_string(),
            renamed_destination: None,
            staged: "staged".to_string(),
            backup: "backup".to_string(),
        })
        .take(super::MAX_REKEY_JOURNAL_ENTRIES + 1)
        .collect(),
    };
    assert!(
        EncryptedWorkspace::validate_rekey_journal(&too_many)
            .unwrap_err()
            .to_string()
            .contains("entries")
    );

    let long_path = RekeyJournal {
        version: REKEY_JOURNAL_VERSION,
        old_key: metadata.clone(),
        new_key: metadata,
        entries: vec![RekeyEntry {
            destination: "x".repeat(super::super::MAX_CONTROL_PATH_BYTES + 1),
            renamed_destination: None,
            staged: "staged".to_string(),
            backup: "backup".to_string(),
        }],
    ...
    assert!(
        EncryptedWorkspace::validate_rekey_journal(&long_path)
            .unwrap_err()
            .to_string()
